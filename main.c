...
void deleteLastRecord(int *arrExistingElementCountPtr, struct StudentDataRecord arr[NO_OF_STUDENT_RECORDS])
{
    ...
    struct StudentDataRecord lastDataRecord = arr[*arrExistingElementCountPtr - 1]; // Get the last record
    ...
    scanf(" %c", &userFeedback); // Use a space before %c to consume any lingering newline

    if(userFeedback == 'Y' || userFeedback == 'y')
    {
        // Reset the last record directly in the array
        strcpy(arr[*arrExistingElementCountPtr - 1].indexNo, "");
        arr[*arrExistingElementCountPtr - 1].maths = 0;
        arr[*arrExistingElementCountPtr - 1].physics = 0;
        arr[*arrExistingElementCountPtr - 1].chemistry = 0;
        arr[*arrExistingElementCountPtr - 1].average = 0.0f;
        arr[*arrExistingElementCountPtr - 1].grade = '\0';
